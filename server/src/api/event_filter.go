...
func (bs *BoardSubscription) eventFilter(event *realtime.BoardEvent, userID uuid.UUID) *realtime.BoardEvent {
	s := make([]*sessions.BoardSession, len(bs.boardParticipants))
	for i, session := range bs.boardParticipants {
		s[i] = &sessions.BoardSession{
	...
	isMod := sessions.CheckSessionRole(userID, s, []common.SessionRole{common.ModeratorRole, common.OwnerRole})
	switch event.Type {
	...
	if isMod {
		...
		updatedSessions := technical_helper.MapSlice(bs.boardParticipants, func(boardSession *FullSession) *FullSession {
			if boardSession.User.ID == participantSession.User {
				return &FullSession{
					User:              users.User{ID: participantSession.User},
...
func eventInitFilter(event InitEvent, clientID uuid.UUID) InitEvent {
	s := make([]*sessions.BoardSession, len(event.Data.BoardSessions))
	...
	isMod := sessions.CheckSessionRole(clientID, s, []common.SessionRole{common.ModeratorRole, common.OwnerRole})
