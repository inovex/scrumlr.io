...
func (s *Server) updateReadyStates(w http.ResponseWriter, r *http.Request) {
	log := logger.FromRequest(r)
	board := r.Context().Value("Board").(uuid.UUID)
	caller := r.Context().Value("User").(uuid.UUID)

	// Get ready state from request body
	var requestBody dto.BoardSessionsUpdateRequest
