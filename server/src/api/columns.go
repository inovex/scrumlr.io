...
func (s *Server) createColumn(w http.ResponseWriter, r *http.Request) {
	...
	if s.basePath == "/" {
		w.Header().Set("Location", fmt.Sprintf("%s://%s/boards/%s/columns/%s", auth.GetProtocol(r), r.Host, board, column.ID))
	} else {
		w.Header().Set("Location", fmt.Sprintf("%s://%s%s/boards/%s/columns/%s", auth.GetProtocol(r), r.Host, s.basePath, board, column.ID))
	}
