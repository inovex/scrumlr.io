	...
	for {
		...
		if err != nil {
			if s.wsService.IsNormalClose(err) {
				...
				s.boardSubscriptionsMu.RLock()
				b := s.boardSubscriptions[id]
				s.boardSubscriptionsMu.RUnlock()
				// this condition is only here to avoid potential nil pointer dereference
				if b != nil {
					b.mu.Lock()
