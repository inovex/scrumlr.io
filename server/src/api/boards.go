...
func (s *Server) createBoard(w http.ResponseWriter, r *http.Request) {
	...
	// build the response
	if s.basePath == "/" {
		w.Header().Set("Location", fmt.Sprintf("%s://%s/boards/%s", auth.GetProtocol(r), r.Host, b.ID))
	} else {
		w.Header().Set("Location", fmt.Sprintf("%s://%s%s/boards/%s", auth.GetProtocol(r), r.Host, s.basePath, b.ID))
	}
...
func (s *Server) joinBoard(w http.ResponseWriter, r *http.Request) {
	...
	if exists {
		...
		if s.basePath == "/" {
			http.Redirect(w, r, fmt.Sprintf("%s://%s/boards/%s/participants/%s", auth.GetProtocol(r), r.Host, board, user), http.StatusSeeOther)
		} else {
			http.Redirect(w, r, fmt.Sprintf("%s://%s%s/boards/%s/participants/%s", auth.GetProtocol(r), r.Host, s.basePath, board, user), http.StatusSeeOther)
		}
		return
	}
	...
	if b.AccessPolicy == boards.Public {
		...
		if s.basePath == "/" {
			w.Header().Set("Location", fmt.Sprintf("%s://%s/boards/%s/participants/%s", auth.GetProtocol(r), r.Host, board, user))
		} else {
			w.Header().Set("Location", fmt.Sprintf("%s://%s%s/boards/%s/participants/%s", auth.GetProtocol(r), r.Host, s.basePath, board, user))
		}
	...
	if b.AccessPolicy == boards.ByInvite {
		...
		if sessionExists {
			if s.basePath == "/" {
				w.Header().Set("Location", fmt.Sprintf("%s://%s/boards/%s/requests/%s", auth.GetProtocol(r), r.Host, board, user))
			} else {
				w.Header().Set("Location", fmt.Sprintf("%s://%s%s/boards/%s/requests/%s", auth.GetProtocol(r), r.Host, s.basePath, board, user))
			}
