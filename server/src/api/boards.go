...
func (s *Server) createBoard(w http.ResponseWriter, r *http.Request) {
  ...
  // build the response
  w.Header().Set("Location", fmt.Sprintf("%s/boards/%s", s.baseURL, b.ID))
...
func (s *Server) joinBoard(w http.ResponseWriter, r *http.Request) {
  ...
  if b.AccessPolicy == types.AccessPolicyPublic {
    ...
    w.Header().Set("Location", fmt.Sprintf("%s/boards/%s/participants/%s", s.baseURL, board, user))
  ...
  if b.AccessPolicy == types.AccessPolicyByInvite {
    ...
    if sessionExists {
      w.Header().Set("Location", fmt.Sprintf("%s/boards/%s/requests/%s", s.baseURL, board, user))
