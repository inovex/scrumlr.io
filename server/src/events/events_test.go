...
func TestOpenBoardSocket(t *testing.T) {
	...
	mockSessionService.EXPECT().Connect(mock.Anything, boardId, userId).
		Return(nil)
...
func TestOpenSessionRequestSocket(t *testing.T) {
	...
	mockSessionRequestConnection := NewMockSessionRequestConnectionManager(t)
	mockSessionRequestConnection.EXPECT().Register(mock.Anything, connection, boardId, userId).
		Return(nil)
	mockSessionRequestConnection.EXPECT().Unregister(mock.Anything, boardId, userId)
