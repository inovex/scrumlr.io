		...
		Commands: []*cli.Command{
			{
				...
				Commands: []*cli.Command{
					...
					{
						Name:      "up",
						Usage:     "Apply all migrations up to the latest if not specified with the --version flag",
...
func getVersion(ctx context.Context, cmd *cli.Command) error {
	...
	defer func() {
		...
		if sourceError != nil {
			err = fmt.Errorf("failed close database connection with err %w", sourceError)
		}

		if databaseError != nil {
			err = fmt.Errorf("failed close database connection with err %w", databaseError)
		}
	}()

	version, migrationFailed, err := m.Version()
	if errors.Is(err, migrate.ErrNilVersion) {
		_, err = fmt.Fprintln(cmd.Writer, "database not yet migrated")
		if err != nil {
			return err
		}
	...
	_, err = fmt.Fprintf(cmd.Writer, "database at migration version %d, all migration applied successfully %t\n", version, !migrationFailed)
	if err != nil {
		return err
	}
...
func migrateUp(ctx context.Context, cmd *cli.Command) error {
	...
	if !cmd.IsSet("version") {
		...
		_, err = fmt.Fprintf(cmd.Writer, "successfully migrated database to version %d\n", dbVersion)
		if err != nil {
			return err
		}
