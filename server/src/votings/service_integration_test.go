...
func (suite *VotingServiceIntegrationTestSuite) SetupTest() {
  seededTestDB := testDbTemplates.NewSeededTestDB(suite.T(), getSeedingForVotingDatabase, votingTestSeedHash)
  votingDB := NewVotingDatabase(seededTestDB)
...
// returns a function that initializes the test database
func getSeedingForVotingDatabase(db *bun.DB) {
