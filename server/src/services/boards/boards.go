...
func (s *BoardService) FullBoard(ctx context.Context, boardID uuid.UUID) (*dto.Board, []*dto.BoardSessionRequest, []*dto.BoardSession, []*dto.Column, []*dto.Note, []*dto.Voting, []*dto.Vote, []*dto.Assign, error) {
	board, requests, sessions, columns, notes, votings, votes, assign, err := s.database.Get(boardID)
	if err != nil {
	...
	return new(dto.Board).From(board), dto.BoardSessionRequests(requests), dto.BoardSessions(sessions), dto.Columns(columns), dto.Notes(notes), dto.Votings(votings, votes), personalVotes, dto.Assignings(assign), err
}
