...
func (s *BoardService) FullBoard(ctx context.Context, boardID uuid.UUID) (*dto.Board, []*dto.BoardSessionRequest, []*dto.BoardSession, *dto.WrappedColumns, []*dto.Note, []*dto.Voting, []*dto.Vote, []*dto.Assignment, error) {
	...
	return new(dto.Board).From(board), dto.BoardSessionRequests(requests), dto.BoardSessions(sessions), dto.WrapColumns(columns), dto.Notes(notes), dto.Votings(votings, votes), personalVotes, dto.Assignments(assignments), err
}
