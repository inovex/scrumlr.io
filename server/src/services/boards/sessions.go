...
func NewBoardSessionService(db *database.Database, rt *realtime.Realtime) services.BoardSessions {
...
func (s *BoardSessionService) Update(_ context.Context, body dto.BoardSessionUpdateRequest) (*dto.BoardSession, error) {
	...
	if body.Role != nil {
		...
		} else if sessionOfCaller.Role == types.SessionRoleOwner && *body.Role != types.SessionRoleOwner {
