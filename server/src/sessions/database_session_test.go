...
func (suite *DatabaseSessionTestSuite) Test_Database_CreateSession_Owner() {
	...
	assert.NotNil(t, err)
	assert.Equal(t, errors.New("not allowed to create board session with owner role"), err)
...
func (suite *DatabaseSessionTestSuite) Test_Database_UpdateSession() {
	t := suite.T()
	database := NewSessionDatabase(suite.db)
	session := DatabaseBoardSession{
		User:      suite.users["Luke"].id,
		Board:     suite.boards["Write"].id,
		Connected: true,
	}

	dbSession, err := database.Update(DatabaseBoardSessionUpdate{Board: session.Board, User: session.User, Connected: &session.Connected})

	assert.Nil(t, err)
	assert.Equal(t, session.User, dbSession.User)
	assert.Equal(t, session.Board, dbSession.Board)
	assert.Equal(t, session.Connected, dbSession.Connected)
}
...
func (suite *DatabaseSessionTestSuite) SeedDatabase(db *bun.DB) {
	...
	// test sessions
	suite.sessions = make(map[string]DatabaseBoardSession, 12)
