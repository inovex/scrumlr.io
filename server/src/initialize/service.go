...
func InitializeColumnsService(db *bun.DB, rt *realtime.Broker) columns.ColumnsService {
	columnsDB := columns.NewColumnsDatabase(db)
	columnsService := columns.NewColumnsService(columnsDB, rt)
