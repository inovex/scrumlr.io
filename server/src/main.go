...
func run(c *cli.Context) error {
  ...
  initializer := initialize.NewServiceInitializer(bun, rt, basePath, r)
  ...
  initializer.InitializeHealthService()

  initializer.InitializeBoardReactionService()
  reactionService := initializer.InitializeReactionService()

  initializer.InitializeBoardTemplateService()
  initializer.InitializeColumnTemplateService()
  ...
  //auth.NewAuthConfiguration(providersMap, unsafeKeyWithNewlines, keyWithNewlines, bun, userService)
  if err != nil {
    return fmt.Errorf("unable to setup authentication: %w", err)
  }

  initializer.InitializeBoardService(sessionRequestService, sessionService, columnService, noteService, reactionService, votingService)
  initializer.InitializeAuthService(feedbackService, providersMap, unsafeKeyWithNewlines, keyWithNewlines, bun, userService, c.Bool("disable-anonymous-login"), c.Bool("allow-anonymous-custom-templates"))
