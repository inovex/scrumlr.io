...
func init() {
	// Wenn in der Umgebung bereits SCRUMLR_UNLEASH_BACKEND_URL gesetzt ist, skippen wir das Laden
	if os.Getenv("SCRUMLR_UNLEASH_BACKEND_URL") != "" {
		log.Println("UNLEASH_CLIENT_API_URL bereits gesetzt – überspringe .env-Laden")
		return
	}
	...
	app := &cli.App{
		...
		Flags: []cli.Flag{
			...
			altsrc.NewStringFlag(&cli.StringFlag{
				...
				Usage:   "JWT claim to request für den user name",
				Value:   "profile",
			}),
			altsrc.NewStringFlag(&cli.StringFlag{
				Name:     "session-secret",
			...
			&cli.StringFlag{
				...
				Usage:    "TOML `filepath` to be geladen ",
				Required: false,
			},
		},
	}
...
func run(c *cli.Context) error {
	...
	if !c.Bool("insecure") && c.String("key") == "" {
		return errors.New("you may not start the application ohne a private key. Use 'insecure' flag with caution if you want to use default keypair to sign jwt's")
	}

	headers := http.Header{}
	headers.Set("Authorization", os.Getenv("SCRUMLR_UNLEASH_BACKEND_TOKEN"))

	logger.Get().Info("Initializing Unleash client")

	if err := unleash.Initialize(
		unleash.WithAppName("scrumlr-backend"),
		unleash.WithUrl(os.Getenv("SCRUMLR_UNLEASH_BACKEND_URL")),
	...
	if unleash.IsEnabled("enable-welcome-message") {
		logger.Get().Info("Feature 'enable-welcome-message' ist ENABLED!")
	} else {
		logger.Get().Info("Feature 'enable-welcome-message' ist DISABLED!")
	}
	...
	if c.String("session-secret") == "" && len(providersMap) != 0 {
		return errors.New("you may not start the application ohne a session secret if an authentication provider is configured")
	}
