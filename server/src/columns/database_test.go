...
func (suite *DatabaseColumnTestSuite) SeedDatabase(db *bun.DB) {
	// test boards
	suite.boards = make(map[string]TestBoard, 4)
	...
	// test columns
	suite.columns = make(map[string]DatabaseColumn, 5)
	// test columns to write
	suite.columns["Update"] = DatabaseColumn{ID: uuid.New(), Board: suite.boards["Write"].id, Name: "Column 1", Description: "This is a description", Color: ColorPlanningPink, Visible: true, Index: 0}
	suite.columns["Update1"] = DatabaseColumn{ID: uuid.New(), Board: suite.boards["Write"].id, Name: "Column 2", Description: "This is a description", Color: ColorYieldingYellow, Visible: true, Index: 1}
	suite.columns["Delete"] = DatabaseColumn{ID: uuid.New(), Board: suite.boards["Write"].id, Name: "Column 3", Description: "This is a description", Color: ColorGoalGreen, Visible: true, Index: 2}
	...
	for _, column := range suite.columns {
		err := databaseinitialize.InsertColumn(db, column.ID, column.Board, column.Name, column.Description, string(column.Color), column.Visible, column.Index)
		if err != nil {
