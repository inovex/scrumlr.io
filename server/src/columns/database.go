...
func (db *DB) Delete(ctx context.Context, affectedBoard, column, user uuid.UUID) error {
	...
	_, err := db.db.NewDelete().
		...
		Where("id = ?", column).
		Returning("*").
		Exec(common.ContextWithValues(ctx, "Database", db, identifiers.BoardIdentifier, affectedBoard, identifiers.ColumnIdentifier, column, identifiers.UserIdentifier, user, "Result", &columns), &columns)
