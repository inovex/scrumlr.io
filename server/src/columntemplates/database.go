...
func (db *DB) Delete(ctx context.Context, board, column, user uuid.UUID) error {
	var columns []DatabaseColumnTemplate
	...
	_, err := db.db.NewDelete().
		...
		Where("id = ?", column).
		Returning("*").
		Exec(common.ContextWithValues(ctx, "Database", db, identifiers.BoardTemplateIdentifier, board, identifiers.ColumnTemplateIdentifier, column, identifiers.UserIdentifier, user, "Result", &columns), &columns)
