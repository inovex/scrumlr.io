...
type SessionRequestDatabase interface {
	...
	Gets(board uuid.UUID, status ...RequestStatus) ([]DatabaseBoardSessionRequest, error)
...
func (service *BoardSessionRequestService) Gets(ctx context.Context, boardID uuid.UUID, statusQuery string) ([]*BoardSessionRequest, error) {
	log := logger.FromContext(ctx)
	...
	requests, err := service.database.Gets(boardID, filters...)
	if err != nil {
