...
func NewNotesDatabase(database *bun.DB) *DB {
	db := DB{db: database}

	return &db
}

func (d *DB) CreateNote(insert NoteInsert) (NoteDB, error) {
	var note NoteDB
...
func (d *DB) ImportNote(insert NoteImport) (NoteDB, error) {
	var note NoteDB
...
func (d *DB) UpdateNote(caller uuid.UUID, update NoteUpdate) (NoteDB, error) {
	...
	noteSelect := d.db.NewSelect().Model((*NoteDB)(nil)).Column("author").Where("id = ?", update.ID).Where("board = ?", update.Board)
...
func (d *DB) updateNoteText(update NoteUpdate) (NoteDB, error) {
	var note NoteDB
...
func (d *DB) updateNoteWithoutStack(update NoteUpdate) (NoteDB, error) {
...
func (d *DB) updateNoteWithStack(update NoteUpdate) (NoteDB, error) {
	newRank := update.Position.Rank
	if update.Position.Rank < 0 {
		newRank = 0
	}
