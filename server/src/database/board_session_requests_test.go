...
func testUpdateOfBoardSessionRequestToAccepted(t *testing.T) {
	...
	_, err := sessionRequestDb.UpdateBoardSessionRequest(sessions.DatabaseBoardSessionRequestUpdate{
		Board:  board.ID,
		User:   user.ID,
		Status: sessions.BoardSessionRequestStatusAccepted,
	})
	assert.Nil(t, err)
...
func testUpdateOfBoardSessionToRejected(t *testing.T) {
	...
	_, err := sessionRequestDb.UpdateBoardSessionRequest(sessions.DatabaseBoardSessionRequestUpdate{
		Board:  board.ID,
		User:   user.ID,
		Status: sessions.BoardSessionRequestStatusRejected,
	})
	assert.Nil(t, err)
}
...
func testUpdateOfRejectedSessionToPendingFails(t *testing.T) {
	...
	_, err := sessionRequestDb.UpdateBoardSessionRequest(sessions.DatabaseBoardSessionRequestUpdate{
		Board:  board.ID,
		User:   user.ID,
		Status: sessions.BoardSessionRequestStatusPending,
	})
	assert.NotNil(t, err)
}
...
func testGetBoardSessionRequest(t *testing.T) {
	...
	assert.Equal(t, sessions.BoardSessionRequestStatusAccepted, request.Status)
}
...
func testGetBoardSessionRequestsWithAcceptedFilter(t *testing.T) {
	board := fixture.MustRow("Board.boardSessionRequestsTestBoard").(*Board)
	requests, err := sessionRequestDb.GetBoardSessionRequests(board.ID, sessions.BoardSessionRequestStatusAccepted)
	assert.Nil(t, err)
	assert.Equal(t, 1, len(requests))
	assert.Equal(t, sessions.BoardSessionRequestStatusAccepted, requests[0].Status)
}

func testGetBoardSessionRequestsWithRejectedFilter(t *testing.T) {
	board := fixture.MustRow("Board.boardSessionRequestsTestBoard").(*Board)
	requests, err := sessionRequestDb.GetBoardSessionRequests(board.ID, sessions.BoardSessionRequestStatusRejected)
	assert.Nil(t, err)
	assert.Equal(t, 1, len(requests))
	assert.Equal(t, sessions.BoardSessionRequestStatusRejected, requests[0].Status)
}

func testGetBoardSessionRequestsWithPendingFilter(t *testing.T) {
	board := fixture.MustRow("Board.boardSessionRequestsTestBoard").(*Board)
	requests, err := sessionRequestDb.GetBoardSessionRequests(board.ID, sessions.BoardSessionRequestStatusPending)
	assert.Nil(t, err)
	assert.Equal(t, 1, len(requests))
	assert.Equal(t, sessions.BoardSessionRequestStatusPending, requests[0].Status)
}

func testGetBoardSessionRequestWithMultipleFilters(t *testing.T) {
	board := fixture.MustRow("Board.boardSessionRequestsTestBoard").(*Board)
	requests, err := sessionRequestDb.GetBoardSessionRequests(board.ID, sessions.BoardSessionRequestStatusAccepted, sessions.BoardSessionRequestStatusRejected)
	assert.Nil(t, err)
