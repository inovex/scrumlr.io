...
func (d *Database) CreateNote(insert NoteInsert) (Note, error) {
	...
	_, err := d.db.NewInsert().
		...
		Returning("*").
		Exec(common.ContextWithValues(context.Background(), "Database", d, identifiers.KeyBoardIdentifier{}, insert.Board), &note)
	return note, err
}
...
func (d *Database) updateNoteText(update NoteUpdate) (Note, error) {
	var note Note
	_, err := d.db.NewUpdate().Model(&update).Column("text").Where("id = ?", update.ID).Where("board = ?", update.Board).Where("id = ?", update.ID).Returning("*").Exec(common.ContextWithValues(context.Background(), "Database", d, identifiers.KeyBoardIdentifier{}, update.Board), &note)
	if err != nil {
		return note, err
	}
	return note, nil
...
func (d *Database) updateNoteWithoutStack(update NoteUpdate) (Note, error) {
	...
	var note []Note
	_, err := query.Exec(common.ContextWithValues(context.Background(), "Database", d, identifiers.KeyBoardIdentifier{}, update.Board), &note)
	return note[0], err
}
	...
	if precondition.Author == caller || precondition.CallerRole == types.SessionRoleModerator || precondition.CallerRole == types.SessionRoleOwner {
		...
		if deleteStack {
			_, err := d.db.NewDelete().
				...
				Exec(common.ContextWithValues(context.Background(), "Database", d, identifiers.KeyBoardIdentifier{}, board, identifiers.KeyNoteIdentifier{}, id, identifiers.KeyUserIdentifier{}, caller, "DeleteStack", deleteStack, "Result", &notes), &notes)
