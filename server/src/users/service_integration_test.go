...
func (suite *UserServiceIntegrationTestsuite) Test_GetMultiple() {
  t := suite.T()
  ctx := context.Background()
  ...
  users, err := userService.GetMultiple(ctx, userIds)
...
func (suite *UserServiceIntegrationTestsuite) Test_GetMultiple_NoUser() {
  t := suite.T()
  ctx := context.Background()
