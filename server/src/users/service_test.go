...
func TestGetMultipleUser(t *testing.T) {
  userIds := []uuid.UUID{uuid.New(), uuid.New(), uuid.New()}
  mockUserDatabase := NewMockUserDatabase(t)
  mockUserDatabase.EXPECT().GetMultipleUsers(mock.Anything, userIds).Return([]DatabaseUser{
    {ID: userIds[0]},
    {ID: userIds[1]},
    {ID: userIds[2]},
  }, nil)
  ...
  user, err := userService.GetMultiple(context.Background(), userIds)
