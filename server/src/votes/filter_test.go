...
func TestVotingNotClosed(t *testing.T) {
  var votes []VoteDB
  voting := buildVoting(uuid.New(), VotingStatusOpen, false)
...
func TestVotingAndVotesIdDiffer(t *testing.T) {
  ...
  voting := buildVoting(uuid.New(), VotingStatusClosed, false)
...
func TestVotingAndVotesIdEqualNoUserDefined(t *testing.T) {
  ...
  voting := buildVoting(voteId, VotingStatusClosed, false)
...
func TestShowVotesOfOthers(t *testing.T) {
  ...
  voting := buildVoting(voteId, VotingStatusClosed, true)
...
func TestCalculateVoteCountForSpecificNote(t *testing.T) {
  ...
  voting := VotingWithVotes([]VotingDB{*buildVoting(voteId, VotingStatusClosed, true)}, []VoteDB{*buildVote(voteId, noteId, userId), *buildVote(voteId, noteId, userId)})[0]
...
func TestShouldReturnNoVotingResultsBecauseVotingIsStillOpen(t *testing.T) {
  ...
  voting := VotingWithVotes([]VotingDB{*buildVoting(voteId, VotingStatusOpen, true)}, []VoteDB{})[0]
  noteSlice := notes.NoteSlice{buildNote(noteId)}
