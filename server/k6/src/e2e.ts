...
import type { CreateBoardTemplateRequest, UpdateBoardTemplateRequest } from "./api/boardTemplate/requests.ts";
import { ColumnClient } from "./api/column/api.ts";
import type { CreateColumnRequest, UpdateColumnRequest } from "./api/column/requests.ts";
import { ColumnTemplateClient } from "./api/columnTemplate/api.ts";
import type { ColumnTemplateRequest } from "./api/columnTemplate/requests.ts";
import { FeedbackClient } from "./api/feedback/api.ts";
import type { CreateFeedbackRequest } from "./api/feedback/requests.ts";
import { HealthClient } from "./api/health/api.ts";
import { InfoClient } from "./api/info/api.ts";
import { NoteClient } from "./api/note/api.ts";
import type { CreateNoteRequest, DeleteNoteRequest, UpdateNoteRequest } from "./api/note/requests.ts";
...
import type { BoardSessionsUpdateRequest, BoardSessionUpdateRequest } from "./api/session/requests.ts";
import { SessionRequestClient } from "./api/sessionRequest/api.ts";
import { UserClient } from "./api/user/api.ts";
import type { UpdateUserRequest } from "./api/user/requests.ts";
...
export default function () {
  ...
  group("Check info", () => {
    ...
    check(response, {
      ...
      "verify info body is defined": () => info !== null && info !== undefined,
    });
  });
  ...
  group("Check login", () => {
    ...
    check(authResponse, {
      ...
      "verify anonymous login body is defined": () => anonymousUser !== null && anonymousUser !== undefined,
    ...
    const invalidNameRequest: AnonymousSignUpRequest = {
      name: "No Name\nHello",
    };
  ...
  group("Check users", () => {
    ...
    const userRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Users-${Date.now()}`,
    };
    ...
    const secondUserRequest: AnonymousSignUpRequest = {
      name: `Stan-Load-Test-${Date.now()}`,
    };
  ...
  group("Check board", () => {
    ...
    const ownerRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Board-Owner-${Date.now()}`,
    };
    const [,] = authClient.loginAnonymous(ownerRequest, ownerContext);

    const participantContext = new http.CookieJar();
    const participantRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Board-Participant-${Date.now()}`,
    };
    ...
    const createBoardRequest: CreateBoardRequest = {
      ...
      columns: [
        {
          name: `E2E-Test-Board-1-${Date.now()}`,
          index: 0,
          color: "backlog-blue",
          visible: true,
        },
        {
          name: `E2E-Test-Board-2-${Date.now()}`,
          index: 1,
          color: "online-orange",
          visible: true,
        },
        {
          name: `E2E-Test-Board-3-${Date.now()}`,
          index: 2,
          color: "goal-green",
          visible: false,
        },
    ...
    const unauthenticatedJoinResponse = boardClient.joinBoard(createdPublicBoard.id, joinRequest, new http.CookieJar(), { "X-Real-IP": getFakeIpAddress(__VU) });
    check(unauthenticatedJoinResponse, {
      "verify unauthenticated join board status is 401": (r) => r.status === 401,
    });

    const [unauthenticatedBoardUsers, unauthenticatedBoardUsersResponse] = boardClient.getBoardUsers(
    ...
    const boardJoinResponse = boardClient.joinBoard(createdPublicBoard.id, joinRequest, participantContext, { "X-Real-IP": getFakeIpAddress(__VU) });
    ...
    const [incrementTimerBoard, ownerIncrementTimerResponse] = boardClient.incrementTimer(
      createdPublicBoard.id,
      ownerContext,
    );
    ...
    check(ownerDeleteTimerResponse, {
      ...
      "verify timer start is cleared": () =>
        deletedTimerBoard?.timerStart === undefined || deletedTimerBoard?.timerStart === null,
      "verify timer end is cleared": () =>
        deletedTimerBoard?.timerEnd === undefined || deletedTimerBoard?.timerEnd === null,
    });
    ...
    const [, participantUpdatedResponse] = boardClient.updateBoard(
      createdPublicBoard.id,
      boardUpdateRequest,
      participantContext,
    );
    ...
    const [updatedBoard, ownerUpdatedResponse] = boardClient.updateBoard(
      createdPublicBoard.id,
      boardUpdateRequest,
      ownerContext,
    );
  ...
  group("Check columns", () => {
    const ownerContext = new http.CookieJar();
    const ownerRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Column-Owner-${Date.now()}`,
    };
    const [,] = authClient.loginAnonymous(ownerRequest, ownerContext);

    const authenticatedContext = new http.CookieJar();
    const authenticatedRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Column-Authenticated-${Date.now()}`,
    };
    const [,] = authClient.loginAnonymous(authenticatedRequest, authenticatedContext);

    const participantContext = new http.CookieJar();
    const participantRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Column-Participant-${Date.now()}`,
    };
    const [,] = authClient.loginAnonymous(participantRequest, participantContext);
    ...
    const createBoardRequest: CreateBoardRequest = {
      ...
      columns: [
        {
          name: `E2E-Test-Column-1-${Date.now()}`,
          index: 0,
          color: "backlog-blue",
          visible: true,
        },
        {
          name: `E2E-Test-Column-2-${Date.now()}`,
          index: 1,
          color: "online-orange",
          visible: true,
        },
        {
          name: `E2E-Test-Column-3-${Date.now()}`,
          index: 2,
          color: "goal-green",
          visible: false,
        },
    ...
    const joinResponse = boardClient.joinBoard(board.id, {}, participantContext, { "X-Real-IP": getFakeIpAddress(__VU) });
    check(joinResponse, {
    ...
    const [, unauthenticatedCreateColumnResponse] = columnClient.createColumn(
      board.id,
      createColumnRequest,
      new http.CookieJar(),
    );
    ...
    const [, authenticatedCreateColumnResponse] = columnClient.createColumn(
      board.id,
      createColumnRequest,
    ...
    const [, participantCreateColumnResponse] = columnClient.createColumn(
      board.id,
      createColumnRequest,
      participantContext,
    );
    ...
    const [createdColumn, ownerCreateColumnResponse] = columnClient.createColumn(
      board.id,
      createColumnRequest,
      ownerContext,
    );
    ...
    const [, unauthenticatedGetColumnResponse] = columnClient.getColumn(
      board.id,
      createdColumn.id,
      new http.CookieJar(),
    );
    ...
    const [participantColumn, participantGetColumnResponse] = columnClient.getColumn(
      board.id,
      createdColumn.id,
      participantContext,
    );
    ...
    const [, unauthenticatedUpdateColumnResponse] = columnClient.updateColumn(
      board.id,
      createdColumn.id,
      updateColumnRequest,
      new http.CookieJar(),
    ...
    const [, authenticatedUpdateColumnResponse] = columnClient.updateColumn(
      board.id,
      createdColumn.id,
      updateColumnRequest,
      authenticatedContext,
    ...
    const [, participantUpdateColumnResponse] = columnClient.updateColumn(
      board.id,
      createdColumn.id,
      updateColumnRequest,
      participantContext,
    ...
    const [updatedColumn, ownerUpdateColumnResponse] = columnClient.updateColumn(
      board.id,
      createdColumn.id,
      updateColumnRequest,
      ownerContext,
    ...
    const unauthenticatedDeleteColumnResponse = columnClient.deleteColumn(
      board.id,
      createdColumn.id,
      new http.CookieJar(),
    );
    ...
    const authenticatedDeleteColumnResponse = columnClient.deleteColumn(
      board.id,
      createdColumn.id,
      authenticatedContext,
    );
  ...
  group("Check notes", () => {
    const ownerContext = new http.CookieJar();
    const ownerRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Note-Owner-${Date.now()}`,
    };
    ...
    const authenticatedContext = new http.CookieJar();
    const authenticatedRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Note-Authenticated-${Date.now()}`,
    };
    ...
    const participantContext = new http.CookieJar();
    const participantRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Note-Participant-${Date.now()}`,
    };
    ...
    const createBoardRequest: CreateBoardRequest = {
      ...
      columns: [
        {
          name: `E2E-Test-Note-1-${Date.now()}`,
          index: 0,
          color: "backlog-blue",
          visible: true,
        },
        {
          name: `E2E-Test-Note-2-${Date.now()}`,
          index: 1,
          color: "online-orange",
          visible: true,
        },
        {
          name: `E2E-Test-Note-3-${Date.now()}`,
          index: 2,
          color: "goal-green",
          visible: true,
        },
    ...
    const [participantNote, participantCreateNoteResponse] = noteClient.createNote(
      board.id,
      participantNoteRequest,
      participantContext,
    );
    ...
    const [getOwnerNote, participantGetOwnerNoteResponse] = noteClient.getNote(
      board.id,
      ownerNote.id,
      participantContext,
    );
    ...
    const [getParticipantNote, ownerGetParticipantNoteResponse] = noteClient.getNote(
      board.id,
      participantNote.id,
      ownerContext,
    );
    ...
    const [, unauthenticatedUpdateNoteResponse] = noteClient.updateNote(
      board.id,
      participantNote.id,
      updateNoteRequest,
      new http.CookieJar(),
    ...
    const [, authenticatedUpdateNoteResponse] = noteClient.updateNote(
      board.id,
      participantNote.id,
      updateNoteRequest,
      authenticatedContext,
    ...
    const [participantUpdatedNote, participantUpdateNoteResponse] = noteClient.updateNote(
      board.id,
      participantNote.id,
      participantUpdateNoteRequest,
      participantContext,
    );
    check(participantUpdateNoteResponse, {
      "verify participant update note status is 200": (r) => r.status === 200,
      "verify participant updated note text matches": () =>
        participantUpdatedNote?.text === participantUpdateNoteRequest.text,
    ...
    const [ownerUpdatedNote, ownerUpdateNoteResponse] = noteClient.updateNote(
      board.id,
      ownerNote.id,
      ownerUpdateNoteRequest,
      ownerContext,
    );
    ...
    const unauthenticatedDeleteNoteResponse = noteClient.deleteNote(
      board.id,
      participantNote.id,
      deleteNoteRequest,
      new http.CookieJar(),
    ...
    const authenticatedDeleteNoteResponse = noteClient.deleteNote(
      board.id,
      participantNote.id,
      deleteNoteRequest,
      authenticatedContext,
    ...
    const participantDeleteNoteResponse = noteClient.deleteNote(
      board.id,
      participantNote.id,
      deleteNoteRequest,
      ownerContext,
  ...
  group("Check votings and votes", () => {
    const ownerContext = new http.CookieJar();
    const ownerRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Voting-Owner-${Date.now()}`,
    };
    const [owner] = authClient.loginAnonymous(ownerRequest, ownerContext);
    if (!owner) {
      return;
    ...
    const authenticatedContext = new http.CookieJar();
    const authenticatedRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Voting-Authenticated-${Date.now()}`,
    };
    const [user] = authClient.loginAnonymous(authenticatedRequest, authenticatedContext);
    if (!user) {
      return;
    ...
    const participantContext = new http.CookieJar();
    const participantRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Voting-Participant-${Date.now()}`,
    };
    const [participant] = authClient.loginAnonymous(participantRequest, participantContext);
    if (!participant) {
      return;
    ...
    const createBoardRequest: CreateBoardRequest = {
      ...
      columns: [
        {
          name: `E2E-Test-Voting-1-${Date.now()}`,
          index: 0,
          color: "backlog-blue",
          visible: true,
        },
        {
          name: `E2E-Test-Voting-2-${Date.now()}`,
          index: 1,
          color: "online-orange",
          visible: true,
        },
        {
          name: `E2E-Test-Voting-3-${Date.now()}`,
          index: 2,
          color: "goal-green",
          visible: true,
        },
    ...
    const [, unauthenticatedCreateVotingResponse] = votingClient.createVoting(
      board.id,
      votingRequest,
      new http.CookieJar(),
    );
    ...
    const [, authenticatedCreateVotingResponse] = votingClient.createVoting(
      board.id,
      votingRequest,
      authenticatedContext,
    );
    ...
    const [participantVoting, participantGetVotingResponse] = votingClient.getVoting(
      board.id,
      voting.id,
      participantContext,
    );
    ...
    const [participantVotes, participantGetVotesResponse] = voteClient.getVotes(
      board.id,
      voting.id,
      null,
      participantContext,
  ...
  group("Check sessions and session requests", () => {
    const ownerContext = new http.CookieJar();
    const ownerRequest: AnonymousSignUpRequest = {
      name: `SessionOwner-${Date.now()}`,
    };
    const [,] = authClient.loginAnonymous(ownerRequest, ownerContext);

    const candidateContext = new http.CookieJar();
    const candidateRequest: AnonymousSignUpRequest = {
      name: `SessionCandidate-${Date.now()}`,
    };
    ...
    const unauthenticatedJoinResponse = boardClient.joinBoard(board.id, {}, new http.CookieJar(), { "X-Real-IP": getFakeIpAddress(__VU) });
    check(unauthenticatedJoinResponse, {
      "verify unauthenticated join board with invite status is 401": (r) => r.status === 401,
    });

    const candidateJoinResponse = boardClient.joinBoard(board.id, {}, candidateContext, { "X-Real-IP": getFakeIpAddress(__VU) });
    ...
    const [, unauthenticatedGetRequestsResponse] = sessionRequestClient.getBoardSessionRequests(
      board.id,
      null,
      new http.CookieJar(),
    );
    ...
    const [ownerSessionRequests, ownerGetResponse] = sessionRequestClient.getBoardSessionRequests(
      board.id,
      null,
      ownerContext,
    );
    ...
    const [, unauthenticatedGetRequestResponse] = sessionRequestClient.getBoardSessionRequest(
      board.id,
      candidate.id,
      new http.CookieJar(),
    );
    ...
    const [candidateGetRequest, candidateGetRequestResponse] = sessionRequestClient.getBoardSessionRequest(
      board.id,
      candidate.id,
      candidateContext,
    );
    ...
    const [, unauthenticatedUpdateResponse] = sessionRequestClient.updateBoardSessionRequest(
      board.id,
      candidate.id,
      { status: "ACCEPTED" },
      new http.CookieJar(),
    ...
    const [, candidateUpdateResponse] = sessionRequestClient.updateBoardSessionRequest(
      board.id,
      candidate.id,
      { status: "ACCEPTED" },
      candidateContext,
    ...
    const [acceptedRequest, ownerUpdateRequestResponse] = sessionRequestClient.updateBoardSessionRequest(
      board.id,
      candidate.id,
      { status: "ACCEPTED" },
      ownerContext,
    ...
    const [, unauthenticatedGetParticipantsResponse] = sessionClient.getParticipants(
      board.id,
      null,
      null,
      null,
    ...
    const [candidateSessions, candidateGetParticipantsResponse] = sessionClient.getParticipants(
      board.id,
      null,
      null,
      null,
    ...
    const [ownerSessions, ownerGetParticipantsResponse] = sessionClient.getParticipants(
      board.id,
      null,
      null,
      null,
    ...
    const [, unauthenticatedGetParticipantResponse] = sessionClient.getParticipant(
      board.id,
      candidate.id,
      new http.CookieJar(),
    );
    ...
    const [candidateSession, candidateGetParticipantResponse] = sessionClient.getParticipant(
      board.id,
      candidate.id,
      candidateContext,
    );
    ...
    const [, unauthenticatedUpdateParticipantResponse] = sessionClient.updateParticipant(
      board.id,
      candidate.id,
      updateSessionReqest,
      new http.CookieJar(),
    ...
    const [updatedSession, participantUpdateResponse] = sessionClient.updateParticipant(
      board.id,
      candidate.id,
      updateSessionReqest,
      candidateContext,
    ...
    const [promotedSession, ownerPromotedResponse] = sessionClient.updateParticipant(
      board.id,
      candidate.id,
      { role: "MODERATOR" },
      ownerContext,
    ...
    const bulkUpdateRequest: BoardSessionsUpdateRequest = {
      ready: false,
      raisedHand: false,
    };
    const [, unauthenticatedBulkUpdateResponse] = sessionClient.updateParticipants(
      board.id,
      bulkUpdateRequest,
      new http.CookieJar(),
    );
    ...
    const [bulkSessions, ownerBulkUpdateResponse] = sessionClient.updateParticipants(
      board.id,
      bulkUpdateRequest,
      ownerContext,
    );
  ...
  group("Check reactions", () => {
    const ownerContext = new http.CookieJar();
    const ownerRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Reaction-Owner-${Date.now()}`,
    };
    const [owner] = authClient.loginAnonymous(ownerRequest, ownerContext);
    if (!owner) {
      return;
    ...
    const createBoardRequest: CreateBoardRequest = {
      ...
      accessPolicy: "PUBLIC",
      columns: [{ name: "Column 1", color: "goal-green", visible: true, index: 0 }],
    };
    const [board] = boardClient.createBoard(createBoardRequest, ownerContext);
    if (!board) {
      return;
    ...
    const [, unauthenticatedCreateReactionResponse] = reactionClient.createReaction(
      board.id,
      reactionRequest,
      new http.CookieJar(),
    );
    ...
    const [createdReaction, ownerCreateReactRes] = reactionClient.createReaction(
      board.id,
      reactionRequest,
      ownerContext,
    );
    ...
    const [, unauthenticatedGetReactionResponse] = reactionClient.getReaction(
      board.id,
      createdReaction.id,
      new http.CookieJar(),
    );
    ...
    const [, unauthenticatedUpdateReactionResponse] = reactionClient.updateReaction(
      board.id,
      createdReaction.id,
      { reactionType: "joy" },
      new http.CookieJar(),
    ...
    const [updatedReaction, ownerUpdateReactionResponse] = reactionClient.updateReaction(
      board.id,
      createdReaction.id,
      { reactionType: "joy" },
      ownerContext,
    ...
    const unauthenticatedDeleteReactionResponse = reactionClient.deleteReaction(
      board.id,
      createdReaction.id,
      new http.CookieJar(),
    );
  ...
  group("Check board reactions", () => {
    const ownerContext = new http.CookieJar();
    const ownerRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Board-Reaction-Owner-${Date.now()}`,
    };
    const [owner] = authClient.loginAnonymous(ownerRequest, ownerContext);
    if (!owner) {
      return;
    ...
    const unauthenticatedBoardReactionResponse = boardReactionClient.createBoardReaction(
      board.id,
      { reactionType: "tada" },
      new http.CookieJar(),
    );
    ...
    const authenticatedBoardReactionResponse = boardReactionClient.createBoardReaction(
      board.id,
      { reactionType: "tada" },
      authenticatedContext,
    );
    ...
    const ownerBoardReactionResponse = boardReactionClient.createBoardReaction(
      board.id,
      { reactionType: "tada" },
      ownerContext,
    );
  ...
  group("Check board and column templates", () => {
    const userContext = new http.CookieJar();
    const userRequest: AnonymousSignUpRequest = {
      name: `Stan-E2E-Test-Template-Authenticated-${Date.now()}`,
    };
    const [user] = authClient.loginAnonymous(userRequest, userContext);
    if (!user) {
      return;
    ...
    const [, unauthenticatedCreateTemplateResponse] = boardTemplateClient.createBoardTemplate(
    ...
    const [createdTemplate, createTemplateResponse] = boardTemplateClient.createBoardTemplate(
      createTemplateRequest,
      userContext,
    );
    ...
    const [, unauthenticatedGetTemplateResponse] = boardTemplateClient.getBoardTemplate(
      createdTemplate.id,
      new http.CookieJar(),
    );
    ...
    const [, unauthenticatedUpdateTemplateResponse] = boardTemplateClient.updateBoardTemplate(
      createdTemplate.id,
    ...
    const [updatedTemplate, updateTemplateResponse] = boardTemplateClient.updateBoardTemplate(
      createdTemplate.id,
      updateTemplateRequest,
      userContext,
    );
    ...
    const [, unauthenticatedCreateColumnTemplateResponse] = columnTemplateClient.createColumnTemplate(
      createdTemplate.id,
      columnTemplateRequest,
      new http.CookieJar(),
    );
    ...
    const [createdColumnTemplate, createColumnTemplateResponse] = columnTemplateClient.createColumnTemplate(
      createdTemplate.id,
      columnTemplateRequest,
      userContext,
    );
    check(createColumnTemplateResponse, {
      ...
      "verify created column template color matches": () =>
        createdColumnTemplate?.color === columnTemplateRequest.color,
    });
    ...
    const [, unauthenticatedGetColumnTemplates] = columnTemplateClient.getColumnTemplates(
      createdTemplate.id,
      new http.CookieJar(),
    );
    ...
    const [columnTemplates, getColumnTemplatesResponse] = columnTemplateClient.getColumnTemplates(
      createdTemplate.id,
      userContext,
    );
    ...
    const [, unauthenticatedGetColumnTemplateResponse] = columnTemplateClient.getColumnTemplate(
      createdTemplate.id,
      createdColumnTemplate.id,
      new http.CookieJar(),
    );
    ...
    const [columnTemplate, getColumnTemplateResponse] = columnTemplateClient.getColumnTemplate(
      createdTemplate.id,
      createdColumnTemplate.id,
      userContext,
    );
    ...
    const [, unauthenticatedUpdateColumnTemplate] = columnTemplateClient.updateColumnTemplate(
      createdTemplate.id,
      createdColumnTemplate.id,
      updateColumnTemplateRequset,
      new http.CookieJar(),
    );
    ...
    const [updatedColumnTemplate, updateColumnTemplateResponse] = columnTemplateClient.updateColumnTemplate(
      createdTemplate.id,
      createdColumnTemplate.id,
      updateColumnTemplateRequset,
      userContext,
    ...
    const unauthenticatedDeleteColumnTemplateResponse = columnTemplateClient.deleteColumnTemplate(
      createdTemplate.id,
      createdColumnTemplate.id,
      new http.CookieJar(),
    );
    ...
    const deleteColumnTemplateResponse = columnTemplateClient.deleteColumnTemplate(
      createdTemplate.id,
      createdColumnTemplate.id,
      userContext,
    );
    ...
    const unauthenticatedDeleteTemplateResponse = boardTemplateClient.deleteBoardTemplate(
      createdTemplate.id,
      new http.CookieJar(),
    );
