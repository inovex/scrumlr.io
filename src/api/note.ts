...
export const NoteAPI = {
  ...
  addNote: async (boardId: string, columnId: string, text: string) => {
    try {
      ...
      throw new Error(`unable to create note with error: ${error}`, {cause: error});
    }
  },
  ...
  deleteNote: async (board: string, noteId: string, deleteStack: boolean) => {
    try {
      ...
      throw new Error(`unable to delete note with error: ${error}`, {cause: error});
    }
  },
  ...
  editNote: async (board: string, note: string, request: EditNote) => {
    try {
      ...
      throw new Error(`unable to update note: ${error}`, {cause: error});
    }
  },
};
