...
describe("<PassphraseDialog />", () => {
  test("snapshot test", () => {
    const {container} = render(<PassphraseDialog onSubmit={vi.fn()} incorrectPassphrase={false} />);
    expect(container.firstChild).toMatchSnapshot();
  });

  test("should disable submit button if no passphrase has been entered", () => {
    const {getByLabelText} = render(<PassphraseDialog onSubmit={vi.fn()} incorrectPassphrase={false} />);
  ...
  test("should toggle the password visibility", () => {
    const {getByDisplayValue, getByLabelText, getByPlaceholderText} = render(<PassphraseDialog onSubmit={vi.fn()} incorrectPassphrase={false} />);
    const passwordInput = getByPlaceholderText(t("PassphraseDialog.inputPlaceholder"));
    fireEvent.change(passwordInput, {target: {value: "Passphrase"}});
