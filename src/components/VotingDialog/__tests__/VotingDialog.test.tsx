...
describe("VotingDialog", () => {
  ...
  xit("should dispatch to store correctly on start voting button click", () => {
    ...
    expect(storeDispatchSpy).toHaveBeenCalledWith(createVoting({voteLimit: 5, showVotesOfOthers: true, allowMultipleVotes: false}));
  });
  ...
  xit("should dispatch to store correctly on start voting button click with custom vote configuration", () => {
    ...
    fireEvent.click(screen.getByTestId("voting-dialog__start-button"));
    expect(storeDispatchSpy).toHaveBeenCalledWith(createVoting({voteLimit: 6, showVotesOfOthers: false, allowMultipleVotes: true}));
  });
  ...
  it("should dispatch to store correctly on stop voting button click", () => {
    ...
    expect(storeDispatchSpy).toHaveBeenCalledWith(closeVoting(getTestApplicationState().votings.open!.id));
  });
});
