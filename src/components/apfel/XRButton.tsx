...
export const XRButton: (props: ButtonProperties & RefAttributes<ComponentInternals>) => ReactNode = forwardRef(
  ({children, size = "md", variant = "rect", platter, selected, disabled, ...props}, ref) => {
    const {borderRadius, fontSize, height, padding, iconSize} = typeof size === "number" ? getAribtrarySize(size) : sizes[size];
    return (
      <Container
