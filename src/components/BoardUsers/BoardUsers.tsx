    ...
    <ul className="board-users">
      ...
      {usersToShow.map((participant) => (
        <li key={participant.user.id}>
          <UserAvatar id={participant.user.id} avatar={participant.user.avatar} ready={participant.ready} name={participant.user.name} />
        </li>
      ))}
      {!!me && <UserAvatar id={me.user.id} avatar={me.user.avatar} ready={me.ready} name={me.user.name} />}
    </ul>
  );
};
