...
export const useBoardGuard = (id: string) => {
  ...
  const { boards, members, pending, cards, columns, settings, users } = useSelector((store: ApplicationState) => store.firestore.data);
  useFirestoreConnect(() => {
            ...
            setState({
              ...
              databaseQueries: [
                ...state.databaseQueries,
                {
                  collection: 'boards',
                  doc: id,
                  subcollections: [{ collection: 'pending', doc: currentUserUid }],
                  storeAs: 'pending',
                },
              ],
            });
