...
if [ -z "$DB_URL" ] || [ -z "$DEPLOYMENT_DOMAIN" ]; then
  echo "Usage: $0 <DB_URL> <DEPLOYMENT_DOMAIN>"
  exit 1
fi
...
DB_UR_SCRUMLR=$(echo "$DB_URL" | sed 's/\/stackit/\/scrumlr/')
kubectl -n scrumlr create secret generic postgresql-creds --from-literal=url="$DB_UR_SCRUMLR" --dry-run=client -o yaml | kubectl apply -f -
...
# check if DB exists
if ! psql "$DB_URL" -lqt | cut -d \| -f 1 | grep -qw scrumlr; then
