...
const app = admin.initializeApp({
  databaseURL: "https://scrumlr-live-eu.europe-west1.firebasedatabase.app"
});
...
const cleanupDatabase = exitFunction => {
  ...
  const query = admin
    .database("https://scrumlr-live-eu.europe-west1.firebasedatabase.app")
    .ref("/boards");
  return query
    .once("value")
    .then(snapshot => {
      snapshot.forEach(childSnapshot => {
        ...
        if (
          !childData.private ||
          !childData.private.config ||
          !childData.private.config.created ||
          !childData.private.users ||
          !childData.private.cards ||
          (new Date(childData.private.config.created).getTime() <
            expirationDate.getTime() &&
            (Object.keys(childData.private.users).length === 1 ||
              Object.keys(childData.private.cards).length === 0))
        ) {
...
exports.scheduledDatabaseCleanup = functions
  .region("europe-west1")
...
exports.detectStackCycle = functions
  .region("europe-west1")
  .database.instance("scrumlr-live-eu")
  .ref("/boards/{boardId}/private/cards/{cardId}")
  .onUpdate((change, context) => {
    ...
    if (newValue.parent === oldValue.parent) {
      ...
      const query = admin
        .database("https://scrumlr-live-eu.europe-west1.firebasedatabase.app")
        .ref(`/boards/${context.params.boardId}/private/cards`);
